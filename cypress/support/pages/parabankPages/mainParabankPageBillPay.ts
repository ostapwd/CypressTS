...
public checkingHowManyInputsThereAreInTheBillyPay() {
    this.checkingHowManyInputsInTheBillyPay().then(($lis) => {
        ...
        return this;
         ...
         public fillAllInputsToEnterPayeeInformation(user) {
            this.fillInputsToEnterPayeeInformation().then(tableRow => {
                ...
                cy.wrap(tableRow).find('input[name="payee.address.street"]')
                .type(user.address)
                ...
                //cy.wrap(tableRow).find('input[ng-model="accountId"]').type(user.fromAccount)
        ...
        public checkingBillPaymentComplete() {
            ...
            //.then(element => {expect(element.text()).to.be.equal ('Bill Payment Complete') })
