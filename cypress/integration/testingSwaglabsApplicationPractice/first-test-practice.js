...
describe ("First test", function () {
   ...
   it("Test 2 'Invalid password'", function () {
       ...
       cy.get('[data-test="error"]').then( label => {
           expect(label.text()).
           to.equal('Epic sadface: Username and password do not match any user in this service');
         });
      });
   ...
   it('Test 7 "Testing Pseudo CSS selector "::after" ', function () {
      ...
      .then((window) => {
          ...
          const fooVal = window.sessionStorage.getItem('content');
          cy.log(fooVal);
          expect(fooVal).to.equal('""');
      });*/
